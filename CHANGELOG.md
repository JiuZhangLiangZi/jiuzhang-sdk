...
- 新增 `jiuzhang.local.applications` 本地应用实验封装，覆盖密集子图、图同构、分子对接和分子振动谱用例。
- 新增四个中文 Jupyter 示例：`dense_subgraph_zh.ipynb`、`graph_isomorphism_zh.ipynb`、`molecular_docking_zh.ipynb`、`molecular_vibronic_spectra_zh.ipynb`。
- 默认安装依赖补齐本地应用实验运行所需的图计算、绘图、分子与采样示例依赖。

### Changed

- README、README_EN 和 PyPI README 更新本地应用实验方法说明，并统一使用 SDK 层表述。
- `sdk_usage.ipynb` 与 `sdk_local_usage.ipynb` 清理为 SDK 层描述。
- 本地应用依赖导入时屏蔽底层兼容性弃用提示，避免 Notebook 显示无关 warning。

## [0.1.1] - 2026-07-29

### Changed

- 默认安装包纳入本地 GBS 数学、采样与 Blackbird / XIR 序列化依赖，统一使用 `pip install jiuzhang-sdk`。
- README、README_EN 与 PyPI README 增加表情符号分段，并同步默认安装说明。
- 移除 `local-gbs` / `local-ir` 可选扩展入口，缺依赖提示统一指向新版默认安装。
